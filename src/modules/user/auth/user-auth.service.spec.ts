...
describe('UserAuthService', () => {
  ...
  describe('login', () => {
    ...
    it('rejects a valid, active ADMIN-role account without ever comparing the password', async () => {
      usersService.findByUsername.mockResolvedValue(
  ...
  describe('refresh', () => {
    ...
    it('calls verifyAndConsumeRefreshToken scoped to Role.USER', async () => {
    ...
    it('revokes all sessions before throwing when the account role was changed to ADMIN', async () => {
      tokenService.verifyAndConsumeRefreshToken.mockResolvedValue({
        sub: 'user-1',
      });
      usersService.findById.mockResolvedValue(buildUser({ role: Role.ADMIN }));
  ...
  describe('logout', () => {
    it('revokes the session scoped to Role.USER', async () => {
